...
const openEditUserForm = (item) => {
  ...
  const role = ROLES.find(
    (roleItem) => roleItem.nombre === item.rol
  );

  const department = DEPARTMENTS.find(
    (departmentItem) =>
      departmentItem.nombre === item.departamento
  );
  ...
  setUserForm({
    ...
    correo: item.correo || "",
    password: "",
    rolId: role ? String(role.id) : "4",
    departamentoId: department
      ? String(department.id)
      : "1",
    nivelSeguridad: String(
      item.nivelSeguridad || 3
    ),
    pais: item.pais || "PERU",
    tipoContrato:
      item.tipoContrato || "INTERNO",
...
const saveUser = async (event) => {
  ...
  const payload = {
    ...
    departamentoId: Number(
      userForm.departamentoId
    ),
    nivelSeguridad: Number(
      userForm.nivelSeguridad
    ),
    pais: userForm.pais,
    tipoContrato: userForm.tipoContrato,
        ...
        <div>
          ...
          <p>
            {editingUser
              ...
              : "Registra un nuevo usuario en SecureDocs."}
          </p>
        </div>
      ...
      >
        ...
        <label>
          ...
          <input
            ...
            placeholder="usuario@securedocs.com"
            required
          />
        </label>
